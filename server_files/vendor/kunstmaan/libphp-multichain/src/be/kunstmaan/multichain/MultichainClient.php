...
class MultichainClient
{
    ...
    private $headers = array(
        'User-Agent: Multichain PHP Client <https://github.com/kunstmaan/libphp-multichain>',
    );
    ...
    public function listStreamKeyItems($stream, $key, $verbose = false, $count = 1, $start = null, $localOrdering = false)
    {
        if(is_null($start)) $start = -$count;
        return $this->jsonRPCClient->execute("liststreamkeyitems", array($stream, $key, $verbose, $count, $start, $localOrdering));
    }

    /**
    * Publishes an item in stream, passed as a stream name, ref or creation txid, with
    * key provided in text form and data-hex in hexadecimal.
    *
    * @param stream $stream
    * @param string $key
    ...
    *
    */
    public function publishStreamItem($stream, $key, $data_hex)
    {
        return $this->jsonRPCClient->execute("publish", array($stream, $key, $data_hex));
    }

    /**
    * Lists items in stream, passed as a stream name, ref or creation txid. Set verbose 
    * to true for additional information about each item’s transaction. Use count 
    * and start to retrieve part of the list only, with negative start values 
    * (like the default) indicating the most recent items. Set local-ordering to true to 
    * order items by when first seen by this node, rather than their order in the chain. 
    * If an item’s data is larger than the maxshowndata runtime parameter, it will be returned
    * as an object whose fields can be used with gettxoutdata.
    *
    * @param stream $stream
    * @param boolean $verbose
    * @param int $count
    * @param int $start
    * @param boolean $localOrdering
    */
    public function listStreamItems($stream, $verbose = false, $count = 1, $start = null, $localOrdering = false)
    {
        if($start == null)  $start = -$count;
        return $this->jsonRPCClient->execute("liststreamitems", array($stream, $verbose, $count, $start, $localOrdering));
    }
